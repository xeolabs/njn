...
(function () {
    ...
    SceneJS.Lights.prototype._initLight = function (index, cfg) {
        ...
        light.pos = cfg.pos ? [
            pos.x != undefined ? pos.x : 0,
            pos.y != undefined ? pos.y : 0,
            pos.z != undefined ? pos.z : 0
        ] : undefined;
        light.dir = cfg.dir ? [
            dir.x != undefined ? dir.x : 0,
            dir.y != undefined ? dir.y : 0,
            dir.z != undefined ? dir.z : 0
        ] : undefined;
        light.attenuation = [
        ...
        if (texturing) {
            ...
            for (var i = 0, len = states.texture.layers.length; i < len; i++) {
                ...
                if (layer.applyTo == "baseColor") {
                    src.push("texel = texture2D(SCENEJS_uSampler" + i + ", vec2(textureCoord.x, 1.0 - textureCoord.y)).rgb;");
                    if (fragmentHooks.textureColor) {
                        src.push("texel=" + fragmentHooks.textureColor + "(texel);");
                    }
                    if (layer.blendMode == "multiply") {
                        src.push("color = color * (SCENEJS_uLayer" + i + "BlendFactor * texel);");
                    } else {
                        src.push("color = ((1.0 - SCENEJS_uLayer" + i + "BlendFactor) * color) + (SCENEJS_uLayer" + i + "BlendFactor * texel);");
                    }
                }
